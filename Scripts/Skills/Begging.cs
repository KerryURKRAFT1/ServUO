        ...
        private class InternalTarget : Target
        {
            ...
            private class InternalTimer : Timer
            {
                ...
                protected override void OnTick()
                {
                	...
                	if (m_Target.Player) // We can't beg from players
                    {
                        number = 500398; // Perhaps just asking would work better.
                    }
                    else if (!m_Target.Body.IsHuman) // Make sure the NPC is human
                    {
                        number = 500399; // There is little chance of getting money from that!
                    }
                    else if (!m_From.InRange(m_Target, 2))
                    {
                        if (!m_Target.Female)
                        {
                    ...
                    else if (!Core.ML && m_From.Mounted) // If we're on a mount, who would give us money? TODO: guessed it's removed since ML
                    {
                    ...
                    {
                        // Face eachother
                        m_From.Direction = m_From.GetDirectionTo(m_Target);
                        m_Target.Direction = m_Target.GetDirectionTo(m_From);

                        m_From.Animate(32, 5, 1, true, false, 0); // Bow
