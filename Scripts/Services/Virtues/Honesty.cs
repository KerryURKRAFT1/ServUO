...
namespace Server.Services.Virtues
{
	public static class Honesty
	{
		...
		private static void OnAfterSave(AfterWorldSaveEventArgs e)
		{
			...
			//if (Enabled )
			if (Enabled && BaseVendor.AllVendors.Count > 0)
			{
				GenerateHonestyItems();
			}
			else
        	{
            Console.WriteLine("Nessun vendor disponibile, impossibile generare oggetti di onestà.");
...
// Aggiungi un controllo di validità per l'indice
        if (x < 0 || y < 0)
        {
            throw new ArgumentException("Gli indici x e y devono essere non negativi.");
        }
				unchecked
				{
					h = (x + map.Width) * y;
