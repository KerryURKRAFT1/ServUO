...
{
	public class BaseVendorLogging
	{
		...
		public static void Initialize()
		{
			...
			try
			{
				...
				m_Output.WriteLine( "Townsperson Logging set to {0}", BaseVendor.Logging.ToString() );
				m_Output.WriteLine();
			}
			catch
			{
		...
		public static void WriteLine( Mobile from, string text )
		{
			try
			{
				m_Output.WriteLine( "{0}: {1}: {2}", DateTime.Now.ToShortTimeString(), BaseVendorLogging.Format( from ), text );
				...
				using ( StreamWriter sw = new StreamWriter( path, true ) )
					sw.WriteLine( "{0}: {1}: {2}", DateTime.Now, BaseVendorLogging.Format( from ), text );
			}
			catch
			{
			}
