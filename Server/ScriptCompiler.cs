...
namespace Server
{
	public static class ScriptCompiler
	{
		...
		public static string GetCompilerOptions(bool debug)
		{
            ...
            #region Mono Compile Error
#if MONO
			AppendCompilerOption(ref sb, "/langversion:7.2");
#else
		...
		public static bool CompileCSScripts(bool debug, bool cache, out Assembly assembly)
		{
...
#endif
            {
				...
				if (Core.Unix)
                {
					parms.CompilerOptions = String.Format( "{0} /nowarn:169,219,414,618 /recurse:Scripts/*.cs", parms.CompilerOptions );
                    files = new string[0];
                }
		...
		public static void Display(CompilerResults results)
		{
			...
			else
			{
				watch.Stop();							
				Utility.PushColor(ConsoleColor.Green);
				Console.WriteLine("done (0 errors, 0 warnings) ({0:F2} seconds)", watch.Elapsed.TotalSeconds );
				Utility.PopColor();
			}
		}
		...
		public static bool Compile(bool debug, bool cache)
		{
			...
			Utility.PushColor(ConsoleColor.Green);
			Console.WriteLine(
				"done ({0} items, {1} mobiles, {2} customs) ({3:F2} seconds)",
				Core.ScriptItems,
				Core.ScriptMobiles,
				Core.ScriptCustoms,
